        ...
        steps = [
            {
                ...
                "command": "sudo XDG_RUNTIME_DIR=/run/user/0 systemctl --user is-active openclaw-gateway && sudo XDG_RUNTIME_DIR=/run/user/0 systemctl --user stop openclaw-gateway && sudo kill $(pgrep -u root -f openclaw-gateway) 2>/dev/null; sleep 2",
                "validation": "! pgrep -u root -f openclaw-gateway",
                "rollback": "sudo XDG_RUNTIME_DIR=/run/user/0 systemctl --user start openclaw-gateway",
                "critical": True,
            },
            {
                "id": 2,
                ...
                "command": f"sudo rm -f /root/.config/systemd/user/openclaw-gateway.service && sudo rm -f /root/.config/systemd/user/default.target.wants/openclaw-gateway.service && sudo XDG_RUNTIME_DIR=/run/user/0 systemctl --user is-active openclaw-gateway",
                "validation": "! sudo test -f /root/.config/systemd/user/openclaw-gateway.service",
                "rollback": f"sudo mkdir -p /root/.config/systemd/user/default.target.wants",
                "critical": True,
            },
            {
                "id": 3,
                ...
                "command": f"mkdir -p {target_openclaw} && sudo cp -r /root/.openclaw/* {target_openclaw}/' && sudo chown -R {target_user}:{target_user} {target_openclaw}",
