...
setup_sudoers() {
    ...
    
    {
        ...
        echo ""
        echo "# LobsterGuard sudoers - restricted permissions (VULN-2 hardened)"
        echo "# Package management (for auto-fix installations)"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/apt-get update, /usr/bin/apt-get install *"
        echo "# Firewall management"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/sbin/ufw, /usr/sbin/iptables"
        echo "# Service management (restricted to specific services)"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl start openclaw*, /usr/bin/systemctl stop openclaw*, /usr/bin/systemctl restart openclaw*, /usr/bin/systemctl enable *, /usr/bin/systemctl disable *, /usr/bin/systemctl daemon-reload"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /sbin/sysctl -w *, /usr/sbin/sysctl -w *"
        echo "# Audit subsystem"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/sbin/auditctl, /usr/sbin/augenrules"
        echo "# File operations (restricted to system config paths)"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/tee /etc/sysctl.d/*, /usr/bin/tee /etc/systemd/system/*, /usr/bin/tee /etc/ufw/*"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/mkdir -p /etc/sysctl.d, /usr/bin/mkdir -p /etc/systemd/system"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/chmod 644 /etc/sysctl.d/*, /usr/bin/chmod 644 /etc/systemd/system/*"
        echo "# User management (for openclaw_user fix)"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/sbin/useradd, /usr/sbin/usermod"
        echo "# Read-only system inspection"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/journalctl, /usr/bin/ss, /usr/bin/loginctl"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/sbin/aa-status, /usr/bin/aa-status"
        echo "$OC_USER ALL=(ALL) NOPASSWD: /usr/bin/fail2ban-client status"
    } > "$SUDOERS_FILE"
